...
const Drawercomponet = () => {
  return (
    <Drawer.Navigator drawerContent={props => <DrewerScreen {...props} />}>
      <Drawer.Screen name="Drawer" component={Accessoriesthree} />
    </Drawer.Navigator>
  );
};
const Accessoriesthree = props => {
  ...
  const {accThree_list, getAccessoriesthree, searchFilter_branch3} =
    useproductContext();
  const {
    addToCart,
    cart,
      ...
      const obj = {
        ...
        scgst: (
          (Number(get_Qty) * Number(get_price) -
            Number(get_Qty) * Number(get_price) * (100 / (100 + 12))) /
          2
        ).toFixed(2),
      };
    ...
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <View
        ...
        <TextInput
          ...
          onChangeText={value => searchFilter_branch3(value)}
        />
      </View>
